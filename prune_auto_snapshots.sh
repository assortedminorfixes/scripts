...
debug() { [ -z "${VERBOSE}" ] || echo "[${BASH_SOURCE}] $@"; }
...
function clean_snapshot_list
{
  ...
  local VOLLIST=( "$(echo "${SNAP_NAMES[*]}" | sed -e 's/@.*//' | sort -u )" )
  local TMP_LIST
  for VOL in ${VOLLIST}
  do
    TMP_LIST=( "${TMP_LIST[@]}" "$(echo "${SNAP_NAMES[*]}" | awk -v VOL="$VOL" 'BEGIN { FS="@"; ORS=""; OFS=""; print VOL "@";}; $1 == VOL { print TRS $2; TRS="," }')" )
  done
...
ZFSOPTS=""
[ "${NOOP}" = "" -a "${VERBOSE}" = "" ] || ZFSOPTS="-${NOOP}${VERBOSE}"
...
then
  ...
  clean_snapshot_list EMPTY_SNAPS
